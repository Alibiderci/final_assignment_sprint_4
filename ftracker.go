...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	result := ((runningCaloriesMeanSpeedMultiplier * speed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return result
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
    // ваш код здесь
	speedMetresPerSecond := meanSpeed(action, duration) * kmhInMsec
	result := ((walkingCaloriesWeightMultiplier * weight + (math.Pow(speedMetresPerSecond, 2) / (height / cmInM)) * walkingSpeedHeightMultiplier * weight) * duration * minInH)
	return result
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	result := (speed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return result
}
